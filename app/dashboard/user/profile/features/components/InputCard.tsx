...
export const InputCard = ({ label, labelText, value }: Props) => {
  const [setProfileUpdateData, clearData] = individualProfileUdateStore(
    (state) => [state.setProfileUpdateData, state.clearData]
  );
    ...
    <div className="flex flex-col">
      ...
      <div className={clsx("flex items-center justify-between py-1 px-1")}>
        <input
          ...
          className="focus:ring-0 border disabled:cursor-not-allowed disabled:bg-[#fafafa] w-full disabled:text-[#e0e0e0] h-[35px] px-4 border-dark/20  text-xs focus:border-dark focus:ring-dark focus:outline-none placeholder:text-xs"
        />
      </div>
    </div>
  );
